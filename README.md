...
**这个方案是纯 AI 辅助、在真实硬件上反复试错调出来的。** 我把过程如实写下来，
因为本项目最有价值的部分不是那 331 行 Python，而是
[`docs/DEBUG_LOG.md`](docs/DEBUG_LOG.md) 里 14 条失败路径 —— 它们几乎全部来自
"AI 给一个假设 → 上板子试 → 失败 → 把报错贴回去 → 换假设"的循环。

> **先说清楚我自己的水平**：我**不是嵌入式或 Linux 专业人士，是个新手**。
> 这些代码不是我"设计"出来的，而是 AI 生成、我在板子上一条条试出来的。
> 因此 —— **如果你觉得哪里不对，或者照着做报错，请优先找专业人士，或让 AI 帮你调试**，
> 不要默认我这里是对的。见 [docs/SCOPE.md](docs/SCOPE.md)。
...
- **作者是新手，代码没有经过专业评审**。它能跑通，只代表在作者这一套硬件上跑通了。
- **所以本文档的定位是"一份被实测过的个人记录"，而不是"一份可信的设计规范"。**
  换手机、换芯片、换 SDCC 版本，结论都可能需要重新验证。
- 遇到问题时：**先读[排错](#排错--troubleshooting)与 [DEBUG_LOG](docs/DEBUG_LOG.md)，
  再考虑请专业人士或让 AI 帮你调试**（把完整报错贴给它）。
...
│   ├── SCOPE.md              使用边界：只验证过 STC89C52，未做 STM32 试验
...
**这个仓库是作者个人自用的记录，整理出来供参考；作者是新手，很希望得到各位的指导。**
如果哪里的做法不专业、绕了远路、或者结论有误，**欢迎直接指正** ——
包括"你这里应该用 XXX"这种方向性建议。
...
- **这是作者个人自用的新手项目，现在整理出来供大家参考。** 作者
  **不是专业人士，是新手**，目标很朴素：**上实验课时不用背电脑**，
  用手机就能给板子烧程序。代码是**纯 AI 辅助调试**出来的
  （AI 生成 + 作者在真机上反复试错）。
- **适用范围只有一种组合**：江协科技 51 开发板（B 站教学视频配套那块）+
  **STC89C52RC/LE52RC** + 板载 **CH340** + Android/Termux 无 root 手机。
- ⚠️ **没有做过 STM32 或任何其他平台的试验。** 本项目与 STM32 的烧录方式
  （SWD/JTAG/DFU）无关，`stcgal` 也不支持 STM32 —— **请勿用本方案烧 STM32**。
  STC12/STC15/STC8、CH341/CH9102/CP2102 等其他芯片同样**未验证**。
- **遇到问题建议找专业人士，或让 AI 帮你调试**（把完整报错贴给它）。
  作者能力有限，**不提供技术支持**，issue/PR 不承诺处理时间或结果。
- 本软件按 **MIT 许可证"原样"提供，不附带任何担保**。烧录是写硬件的操作，
  接错线（尤其**不共地**）或供电不稳可能损坏设备，**风险与后果由使用者自行承担**。
- 仅供**学习、个人开发与个人自用**。

**欢迎指导。** 这个仓库的主要目的是**把自己踩过的坑记录下来，给后来的人省点时间** ——
如果哪里的做法不专业、有更好的写法、或者结论有误，**非常欢迎指正**，
请直接开 issue 或 PR（见 [CONTRIBUTING.md](CONTRIBUTING.md)）。
只是请理解：作者能力有限，**回复和合并都不承诺时间**。

完整的使用边界、性能限制与风险说明见 **[docs/SCOPE.md](docs/SCOPE.md)**。
